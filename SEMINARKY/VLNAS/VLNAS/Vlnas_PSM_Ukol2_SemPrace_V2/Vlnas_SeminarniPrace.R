...
summary(sat)
# Multiple R-squared:  0.6834 <- model vysvětluje 68.34% variability dat


# Test normality residui <- test předpokladů
shapiro.test(residuals(sat))
# H0 - residua mají normální rozdělení
# H1 - residua NEMAJÍ normální rozdělení
# p-value = 0.0006368 -> zamítáme H0 -> residua nemají normální rozdělení

# Test stability rozptylu <- test předpokladů
library(lmtest)
bptest(sat)
# H0 - rozptyl má normální rozdělení
# H1 - rozptyl NEMÁ normální rozdělení
# p-value = 0.02378 < 0.05 -> zamítáme H0 -> rozptyl NEMÁ normální rozdělení

coef(sat)

#(Intercept)    seriousness   exp_products        service range_products       benefits    investments          price 
#-0.26116368     0.19730604     0.12320323    -0.08820927     0.07889857     0.27746238     0.22008636     0.22419010 

#Intercept (-0.26116368): Když jsou všechny ostatní proměnné nulové, očekává se, že spokojenost bude na této úrovni.

#Seriousness (0.19730604): Zvýšení seriousness o jednu jednotku vede k očekávanému zvýšení celkové spokojenosti o 0.197 jednotek, při ostatních proměnných konstantních.

#Exp_products (0.12320323): Zvýšení exp_products o jednu jednotku vede k očekávanému zvýšení celkové spokojenosti o 0.123 jednotek.

#Service (-0.08820927): Zvýšení service o jednu jednotku vede k očekávanému snížení celkové spokojenosti o 0.088 jednotek. Tento negativní koeficient může naznačovat, že vyšší hodnota této proměnné je vnímána negativně nebo může být v modelu nějaká kolinearita.

#Range_products (0.07889857): Zvýšení range_products o jednu jednotku vede k očekávanému zvýšení celkové spokojenosti o 0.079 jednotek.

#Benefits (0.27746238): Zvýšení benefits o jednu jednotku vede k očekávanému zvýšení celkové spokojenosti o 0.277 jednotek => Jeden z nejsilnějších vlivů v modelu.

#Investments (0.22008636): Zvýšení investments o jednu jednotku vede k očekávanému zvýšení celkové spokojenosti o 0.220 jednotek.

#Price (0.22419010): Zvýšení price o jednu jednotku vede k očekávanému zvýšení celkové spokojenosti o 0.224 jednotek.
#---------------------------------------------------------------------------
...
summary(rec)
# Multiple R-squared:  0.4927 <- model vysvětluje 49.27% variability dat

# Test normality residui <- test předpokladů
shapiro.test(residuals(rec))
# H0 - residua mají normální rozdělení
# H1 - residua NEMAJÍ normální rozdělení
# p-value = 3.913e-09 -> zamítáme H0 -> residua nemají normální rozdělení
...
# H0 - rozptyl má normální rozdělení
# H1 - rozptyl NEMÁ normální rozdělení
# p-value = 0.2038 > 0.05 -> NEzamítáme H0 -> rozptyl má normální rozdělení

coef(rec)
#(Intercept) trustworthiness       solidness    exp_services   qual_products   qual_personal        benefits     investments           price 
#-1.1029460       0.2429200      -0.1432136      -0.1092315       0.1430740       0.3275087       0.1746900       0.2060252       0.2655918 

#Intercept (-1.1029460): Když jsou všechny ostatní proměnné nulové, očekává se, že úroveň doporučení bude na této úrovni.

#Trustworthiness (0.2429200): Zvýšení trustworthiness o jednu jednotku vede k očekávanému zvýšení úrovně doporučení o 0.243 jednotek, při ostatních proměnných konstantních.

#Solidness (-0.1432136): Zvýšení solidness o jednu jednotku vede k očekávanému snížení úrovně doporučení o 0.143 jednotek.

#Exp_services (-0.1092315): Zvýšení exp_services o jednu jednotku vede k očekávanému snížení úrovně doporučení o 0.109 jednotek.

#Qual_products (0.1430740): Zvýšení qual_products o jednu jednotku vede k očekávanému zvýšení úrovně doporučení o 0.143 jednotek.

#Qual_personal (0.3275087): Zvýšení qual_personal o jednu jednotku vede k očekávanému zvýšení úrovně doporučení o 0.328 jednotek. Toto je jeden z nejsilnějších vlivů v modelu.

#Benefits (0.1746900): Zvýšení benefits o jednu jednotku vede k očekávanému zvýšení úrovně doporučení o 0.175 jednotek.

#Investments (0.2060252): Zvýšení investments o jednu jednotku vede k očekávanému zvýšení úrovně doporučení o 0.206 jednotek.

#Price (0.2655918): Zvýšení price o jednu jednotku vede k očekávanému zvýšení úrovně doporučení o 0.266 jednotek.
...
summary(loy)
# Multiple R-squared:  0.3318 <- model vysvětluje 33.18% variability dat

# Test normality residui <- test předpokladů
shapiro.test(residuals(loy))
# H0 - residua mají normální rozdělení
# H1 - residua NEMAJÍ normální rozdělení
# p-value = 4.952e-07 -> zamítáme H0 -> residua nemají normální rozdělení
...
# H0 - rozptyl má normální rozdělení
# H1 - rozptyl NEMÁ normální rozdělení
# p-value = 0.03407 < 0.05 -> zamítáme H0 -> rozptyl NEMÁ normální rozdělení

coef(loy)
#(Intercept)      solidness   exp_products        service        quality  qual_products range_products  qual_personal    investments 
#1.0779584      0.3205259     -0.3121027      0.1529307     -0.1634728      0.3686655     -0.1524164      0.2660578      0.3729120 

#Intercept (1.0779584): Když jsou všechny ostatní proměnné nulové, očekává se, že úroveň loajality bude na této úrovni.

#Solidness (0.3205259): Zvýšení solidness o jednu jednotku vede k očekávanému zvýšení úrovně loajality o 0.321 jednotek, při ostatních proměnných konstantních.

#Exp_products (-0.3121027): Zvýšení exp_products o jednu jednotku vede k očekávanému snížení úrovně loajality o 0.312 jednotek.

#Service (0.1529307): Zvýšení service o jednu jednotku vede k očekávanému zvýšení úrovně loajality o 0.153 jednotek.

#Quality (-0.1634728): Zvýšení quality o jednu jednotku vede k očekávanému snížení úrovně loajality o 0.163 jednotek.

#Qual_products (0.3686655): Zvýšení qual_products o jednu jednotku vede k očekávanému zvýšení úrovně loajality o 0.369 jednotek. Toto je jeden z nejsilnějších vlivů v modelu.

#Range_products (-0.1524164): Zvýšení range_products o jednu jednotku vede k očekávanému snížení úrovně loajality o 0.152 jednotek.

#Qual_personal (0.2660578): Zvýšení qual_personal o jednu jednotku vede k očekávanému zvýšení úrovně loajality o 0.266 jednotek.

#Investments (0.3729120): Zvýšení investments o jednu jednotku vede k očekávanému zvýšení úrovně loajality o 0.373 jednotek.
...
kvalita <- data.frame(exp_services, service, qual_services, qual_overall, quality)

par(mfrow=c(1,1))

boxplot(kvalita, 
        main = "Kvalita služeb a produktů",
        ylab = "Hodnocení",
        col = rainbow(ncol(kvalita)),
        las = 2)

# Zákaznická spokojenost a očekávání
spokojenost <- data.frame(satisfaction, expectations, benefits, performance, comparison)

boxplot(spokojenost, 
        main = "Zákaznícká spokojennost a očekávání",
        ylab = "Hodnocení",
        col = rainbow(ncol(kvalita)),
        las = 2)

# Důvěra
duvera <- data.frame(trustworthiness, seriousness, solidness)
...
# Loyalita
loyalita <- data.frame(loyalty, return, switch, recommendation)
...
# Kvalita produktu
produkt <- data.frame(exp_products, qual_products)

boxplot(produkt, 
        main = "Kvalita produktu",
        ylab = "Hodnocení",
        col = rainbow(ncol(kvalita)),
        las = 2)
